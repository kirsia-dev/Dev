        ...
        local success, err = pcall(function()
            ...
            if npcRoot and root then
                if isSeaBeastTarget then
                    ...
                    local sbDist = tonumber(Options.SeaBeastDistance and Options.SeaBeastDistance.Value) or tonumber(Options.Distance and Options.Distance.Value) or 12
                    local sbType = Options.SeaBeastFarmType and Options.SeaBeastFarmType.Value or "Behind"
                    ...
                    local sbBack = tonumber(Options.SeaBeastParkBack and Options.SeaBeastParkBack.Value) or sbDist
                    local sbHeight = tonumber(Options.SeaBeastParkHeight and Options.SeaBeastParkHeight.Value) or 28
                    local desiredPos
                    if parkMode then
                        -- Keep a safer minimum distance from Sea Beast body to reduce incoming damage.
                        sbBack = math.max(sbBack, 20)
                        desiredPos = (seaAnchor.CFrame * CFrame.new(sbSide, sbHeight, sbBack)).Position
                    elseif sbType == "Above" then
                        desiredPos = seaAnchor.Position + Vector3.new(0, sbDist, 0)
                    elseif sbType == "Below" then
                        desiredPos = seaAnchor.Position + Vector3.new(0, -sbDist, 0)
                    else
                        desiredPos = (seaAnchor.CFrame * CFrame.new(0, 0, sbDist)).Position
                    end
                    ...
                    root.CFrame = CFrame.lookAt(desiredPos, seaAnchor.Position)
                    root.AssemblyLinearVelocity = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero
                ...
                if isSeaBeastTarget and (Status.Main.SeaBeastDodgeUntil or 0) > tick() then
                    ...
                    local evadePos = npcRoot.Position + Vector3.new(0, math.clamp(dodgeHeight, 10, 80), 0)
                    root.CFrame = CFrame.lookAt(evadePos, npcRoot.Position)
                    root.AssemblyLinearVelocity = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero
                    return
                end
