...
local function ApplyVoraTheme()
    local VoraScheme = {
        -- Mapping from `source for vorahub/lib/lib.lua`
    ...
    local ok, err = pcall(function()
        ...
        -- Keep Linoria string keys ("Dark", "Red", "White") in sync for outlines / templates.
        Library.Scheme.Dark = VoraScheme.DarkColor
        Library.Scheme.Red = VoraScheme.RedColor
        Library.Scheme.White = VoraScheme.WhiteColor

        -- Do not call ColorPicker:SetValueRGB here: ThemeManager hooks OnChanged -> ThemeUpdate ->
        -- UpdateColorsUsingRegistry, which can re-enter the UI library deeply enough to C stack overflow
        -- when combined with Notify/New/FillInstance. Scheme + UpdateColorsUsingRegistry is enough.
...
-- Cobalt combo (per Heartbeat, saat toggle ON): (0,0,0),0,false lalu NaN, math.huge, true
...
-- Selaras ReplicatedStorage.DashModule + DashClient: arah (kamera WASD → MoveDirection → look XZ), jarak = CheckDashPath, arg3 = backward
...
-- Pakai Module game bila ada → jarak FireServer float sama performDash (mis. 30.665), bukan salinan yang beda
...
-- Sama DashClient: GetDashDirection(character, humanoid) + getMoveDirection internal
...
-- applyDashMovement-style: BodyVelocity sering diabaikan game baru → pakai AssemblyLinearVelocity tiap Heartbeat (seperti impulse dash)
...
local function DashClient_ApplyDashMovement(hrp, humanoid, dirUnit, pathDistance)
    ...
    -- Cadangan: beberapa tempat masih merespons BodyVelocity
...
-- Seperti performDash di DashClient: anim (ForwardDashAnims / BackwardDashAnim) + sfx "Dash" (SoundService pool → id ini di dump)
...
-- Jika Instant TP bypass ON: farm pakai snap CFrame (menggantikan opsi Tween di dropdown).
...
local function ExecuteFarmLogic(target, island, farmType)
    ...
    -- Boss (and similar) moves often; the farm loop runs every frame and was starting a new tween each time
    -- without cancelling the old one, which stacks tweens and looks like flying around. Only retween when
    -- we're meaningfully far from the desired offset, and cancel any in-flight tween before starting another.
            ...
            if root then
                ...
                if FarmUsesTeleportMovement() then
                    ...
                    if distTo > moveDeadzone then
                        ...
                        -- Keep orientation locked to target to avoid tiny physics drift while inside deadzone.
                        root.CFrame = CFrame.lookAt(root.Position, targetPos)
                    end
                end

                -- Stabilize character so dungeon farm does not feel like falling/stuttering.
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
...
GB.Webhook.Left.Config:AddToggle("WebhookDisconnected", {
    Text = "Webhook Disconnected",
    Default = true,
    Disabled = not Support.Webhook,
})
...
TB_Tabs.Autofarm.T4:AddToggle("InstantTPBypass", {
    Text = "Instant TP bypass",
    Default = false,
})

Toggles.InstantTPBypass:OnChanged(function(v)
    ACThing(v)
end)
...
local dungeonCompletedCount = 0
local dungeonClearLatched = false

task.spawn(function()
    while task.wait(0.35) do
        local dungeonUI = PGui:FindFirstChild("DungeonUI")
        local content = dungeonUI and dungeonUI:FindFirstChild("ContentFrame")
        local waveFrame = content and content:FindFirstChild("WaveFrame")
        local holder = waveFrame and waveFrame:FindFirstChild("Holder")
        local waveLabel = holder and holder:FindFirstChild("TotalWavesAndCurrentWaveYouAreAt")

        if not (DungeonCount and waveLabel and waveLabel:IsA("TextLabel")) then
            continue
        end

        local waveText = tostring(waveLabel.Text or "")
        local isCleared = waveText:upper():find("DUNGEON%s+CLEARED%s*!") ~= nil

        if isCleared then
            if not dungeonClearLatched then
                dungeonCompletedCount = dungeonCompletedCount + 1
                dungeonClearLatched = true
            end
        else
            dungeonClearLatched = false
        end

        local currentWave, totalWave = waveText:match("(%d+)%s*/%s*(%d+)")
        currentWave, totalWave = tonumber(currentWave), tonumber(totalWave)
        if currentWave and totalWave then
            DungeonCount:SetText(("Dungeon Completed: %d | Wave: %d/%d"):format(dungeonCompletedCount, currentWave, totalWave))
        elseif isCleared then
            DungeonCount:SetText(("Dungeon Completed: %d | Wave: CLEARED"):format(dungeonCompletedCount))
        else
            DungeonCount:SetText(("Dungeon Completed: %d | Wave: N/A"):format(dungeonCompletedCount))
        end
    end
end)
...
TB_Tabs.Dungeon.T2:AddSlider("DungeonFarmDistance", {
    Text = "Dungeon Farm Distance",
    Default = 10,
    Min = 3,
    Max = 40,
    Rounding = 1
...
TB_Tabs.Dungeon.T2:AddDropdown("DungeonFarmType", {
    Text = "Dungeon Farm Position",
    Values = {"Behind", "Above", "Below"},
    Default = "Behind",
    Multi = false,
    AllowNull = false,
    ...
    GB.Player.Left.General:AddToggle("FreezeCharacter", { Text = "Freeze Character", Default = false })
    GB.Player.Left.General:AddToggle("AntiKnockback", {
    Text = "Anti Knockback",
    Default = false,
    })
    GB.Player.Left.General:AddToggle("AutoDash", {
        Text = "Auto Dash",
        Default = false,
    })
    GB.Player.Left.General:AddToggle("AutoDashFX", {
        Text = "Auto Dash FX (anim + sound)",
        Default = true,
    })
    GB.Player.Left.General:AddSlider("AutoDashCD", {
        Text = "Dash cooldown (sec)",
        Default = 0.4,
        Min = 0.15,
        Max = 1.25,
        Rounding = 2,
    ...
    GB.Player.Left.General:AddSlider("AutoDashDistance", {
        Text = "Dash max distance",
        Default = 33,
        Min = 15,
        Max = 100,
        Rounding = 0,
    ...
    Toggles.AutoDash:OnChanged(function(v)
        if not v then
            pcall(DashClient_StopAutoDashPhysics)
        end
    end)
    ...
    GB.Player.Right.Game:AddButton({
        Text = "Load Fly GUI",
        Func = function()
            pcall(function()
...
TB_Tabs.Waypoint.T1:AddButton({
    Text = "Set Spawn (Selected Island)",
    Func = function()
        local selectedIsland = Options.SelectedIsland and Options.SelectedIsland.Value
        if not selectedIsland then
            Library:Notify("Please select island first.", 2)
            return
        end

        local crystal = IslandCrystals[selectedIsland]
        if not crystal then
            Library:Notify("Spawn crystal not found for: " .. tostring(selectedIsland), 3)
            return
        end
        ...
        task.wait(0.1)
        root.CFrame = oldCF
        Library:Notify("Spawn set to: " .. tostring(selectedIsland), 3)
    end
})
...
Options.SelectedDiff:OnChanged(function()
    VoteDungeonDifficultyOnce()
end)
...
Toggles.AutoDiff:OnChanged(function(state)
    ...
    Thread("AutoDungeonDiff", Func_AutoDungeonDifficulty, state)
end)
...
Toggles.AutoReplay:OnChanged(function(state)
    ...
    Thread("AutoReplay", Func_AutoReplay, state)
end)
...
Toggles.DungeonAutofarm:OnChanged(function(state)
    Thread("DungeonAutofarm", Func_DungeonAutofarm, state)
end)
...
Toggles.FreezeCharacter:OnChanged(function(v)
    if not v and Shared.FreezeApplied then
        local char = Plr.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            root.Anchored = false
        end
        Shared.FreezeApplied = false
    end
end)
...
Connections.Player_General = RunService.Stepped:Connect(function()
    ...
    if Hum then
        ...
        if Toggles.AutoDash and Toggles.AutoDash.Value and _DR and Hum.Health > 0 and not Hum.Sit and char and root
            and not (Toggles.FreezeCharacter and Toggles.FreezeCharacter.Value) then
            local moving = Hum.MoveDirection.Magnitude > 0.08
            local wasd = UIS:IsKeyDown(Enum.KeyCode.W) or UIS:IsKeyDown(Enum.KeyCode.S)
                or UIS:IsKeyDown(Enum.KeyCode.A) or UIS:IsKeyDown(Enum.KeyCode.D)
            if moving or wasd then
                local dir = Dash_GetDirection(char, Hum, root)
                if dir then
                    local mod = getGameDashModule()
                    local cfg = mod and mod.Config
                    local cap = (cfg and cfg.DashDistance) or 33
                    local raw = tonumber(Options.AutoDashDistance and Options.AutoDashDistance.Value) or cap
                    local maxDist = math.clamp(math.max(raw, 15), 15, 120)
                    maxDist = math.min(maxDist, cap)
                    local okPath, dist = Dash_CheckPath(char, dir, maxDist)
                    if okPath and dist > 0 then
                        local interval = tonumber(Options.AutoDashCD and Options.AutoDashCD.Value)
                            or (cfg and cfg.DashCooldown)
                            or 0.4
                        local now = tick()
                        if now - Shared.AutoDashLast >= interval then
                            Shared.AutoDashLast = now
                            local backward = Dash_IsBackward(char, dir)
                            pcall(function()
                                if Toggles.AutoDashFX and Toggles.AutoDashFX.Value then
                                    DashClient_PlayDashEffects(Hum, root, backward)
                                end
        ...
        if root then
            if Toggles.FreezeCharacter and Toggles.FreezeCharacter.Value then
                root.AssemblyLinearVelocity = Vector3.zero
                root.AssemblyAngularVelocity = Vector3.zero
                root.Anchored = true
                Shared.FreezeApplied = true
            elseif Shared.FreezeApplied then
                root.Anchored = false
                Shared.FreezeApplied = false
            end
        end
...
-- After physics, clear HRP velocity while farm tween runs so gravity does not stack and cause dropping/bobbing.
RunService.Heartbeat:Connect(function()
    if not (Shared.Farm and Shared.Target) then return end
    local char = GetCharacter()
    local root = char and char:FindFirstChild("HumanoidRootPart")
    local tw = Shared.FarmMoveTween
    if root and tw and tw.PlaybackState == Enum.PlaybackState.Playing then
        root.AssemblyLinearVelocity = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
    end
end)
...
task.spawn(function()
    while getgenv().vorahub_Running do
...
ThemeManager:SetFolder("vorahub")
SaveManager:SetFolder("vorahub/SailorPiece")
...
local function ReadClipboardText()
    ...
    if typeof(getclipboard) == "function" then
        getter = getclipboard
    elseif typeof(clipboard) == "function" then
        getter = clipboard
    end
...
local ClipboardConfigGroup = Tabs.Config:AddRightGroupbox("Clipboard Import", "clipboard")

ClipboardConfigGroup:AddInput("ClipboardConfigName", {
    Default = "",
    Numeric = false,
    Finished = true,
    Text = "Config Name (optional)",
    Placeholder = "Example: vorahub_auto"
})

ClipboardConfigGroup:AddButton({
    Text = "Import Config from Clipboard",
    Func = function()
        local clipboardText = ReadClipboardText()
        if not clipboardText or clipboardText == "" then
            Library:Notify("Clipboard is empty or unreadable.", 4)
            return
        end

        local manualName = ""
        if Options.ClipboardConfigName and Options.ClipboardConfigName.Value then
            manualName = TrimText(Options.ClipboardConfigName.Value)
        end
        ...
        if configName == "" then
            Library:Notify("Invalid config name from clipboard.", 4)
            return
        end
        ...
        if not ok then
            Library:Notify("Failed to import config: " .. tostring(err), 5)
            return
        end

        ActiveConfigName = configName
        Library:Notify("Imported config: " .. configName, 4)
    end,
    Disabled = not Support.ClipboardRead,
    DisabledTooltip = "Missing function: getclipboard/clipboard"
})
...
local function SetupConfigAutoPersistence()
    local autoloadName = SaveManager:GetAutoloadConfig()

    if not autoloadName or autoloadName == "" or autoloadName == "none" then
        autoloadName = "vorahub_auto"
        pcall(function()
            SaveManager:Save(autoloadName)
            SaveManager:SaveAutoloadConfig(autoloadName)
        end)
    else
        SaveManager:LoadAutoloadConfig()
    end

    ActiveConfigName = autoloadName

    task.spawn(function()
        while getgenv().vorahub_Running do
            task.wait(20)
            if ActiveConfigName and ActiveConfigName ~= "" and ActiveConfigName ~= "none" then
                pcall(function()
                    SaveManager:Save(ActiveConfigName)
                end)
            end
        end
    end)
